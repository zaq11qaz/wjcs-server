...
public class RechargeRecordServiceImpl extends BaseFrameworkServiceImpl<RechargeRecordEntity, Long> implements RechargeRecordService {
    ...
    public ResultParam updateInviteCouponCash(RechargeRecordEntity entity, HttpServletRequest request, HttpServletResponse response) {
        try {
            entity.setRcharge_type("coupon_177");
            entity.setIs_settlement(false);
            entity.setFinished(true);
            entity.setStatus(2);
            List<RechargeRecordEntity> list = mapper.queryNeedSettlementList(entity);
            if (list != null && list.size() > 0) {
                for (RechargeRecordEntity rechargeRecordEntity : list) {
                    UserInfoEntity userInfoEntity = userInfoMapper.queryByInviteCode(rechargeRecordEntity.getInvite_code());
                    userInfoEntity.setCash(userInfoEntity.getCash().add(new BigDecimal("50")));
                    Integer integer = userInfoMapper.updateCash(userInfoEntity);
                    rechargeRecordEntity.setIs_settlement(true);
                    mapper.updateByPrimaryKeySelective(rechargeRecordEntity);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
