...
//                            commodityCouponEntity.setType("classexperience");
                                ...
                                ResultParam resultParam1 = userCouponService.insertUserCoupon177(entities.get(0));
                            } else if (entities.get(0).getRcharge_type().equalsIgnoreCase("coupon_77")) {
                                ResultParam resultParam1 = userCouponService.insertUserCoupon77(entities.get(0));
                            } else if (entities.get(0).getRcharge_type().equalsIgnoreCase("deductions_coupon")) {
                                ResultParam resultParam1 = userCouponService.insertUserCoupon99(entities.get(0));
...
//            throw new DataException(e.getMessage() == null ? "调用微信错误" : e.getMessage());
     ...
     */
    @Override
    public String aliTransfer(RechargeRecordEntity entity, HttpServletRequest request, HttpServletResponse response) throws
            DataException {
        String string = null;
        try {
            ...
            Boolean b = userInfoService.checkIsCanPay(rechargeInfoEntity.getUser_id(), entity.getAmount());
            if (b) {
...
//                param.setAppid(wxPayParam.getAppId());
                //mch_appid
                param.setMch_appid(wxPayParam.getAppId());
                //mch_id
                param.setMchid(wxPayParam.getMachId());
                //device_info
                param.setDevice_info("WEB");
                String randoms = CommonUtils.generateRandom(30, false, false, false,
                        false);
                ...
                param.setNonce_str(randoms);
                //amount
                param.setAmount(entity.getAmount().multiply(new BigDecimal("100")).longValue());
                //out_trade_no
                param.setOut_trade_no(flowing_no);
                //partner_trade_no
                param.setPartner_trade_no(randoms);
                //check_name
                param.setCheck_name("NO_CHECK");
                //wxopen_id
                param.setOpenid(rechargeInfoEntities.get(0).getWxPay());
                //desc
                param.setDesc("外教超市提现");
                //ip
                param.setSpbill_create_ip(ipAddr);
                Map<String, Object> map = JSONUtils.obj2Map(param, null);
                //sign
                ...
                try {
                    ...
                    e.printStackTrace();
                    logger.info("PayServiceImpl    wxTransfer ");
                }
            }
        }
