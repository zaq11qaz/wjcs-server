package com.huihe.eg.user.service.impl.sms;
...
import com.huihe.eg.user.model.SmsParam;
...
public class SmsService {
    ...
    public ResultParam sendSms(String mobile, String type) {
        ...
        if ("binding".equalsIgnoreCase(type)) {
            UserEntity entity = new UserEntity();
            entity.setLogin_name(mobile);
            entity.setPageSize(1);
            List<UserEntity> userEntities = userMapper.query(entity);
        ...
        if (!"0".equals(select.get(0).text().substring(0, 1))) {
     ...
     * @return
     */
    public ResultParam sendPassSms(String mobile, String content) {
        String msg = param.getSmsi().replace("{content}", content);
        SmsSendRequest smsSingleRequest = new SmsSendRequest(param.getSname(), param.getSpwd(), param.getPsd(), msg, mobile, "");
        String param1 = "";
        Map map = JSON.parseObject(JSON.toJSON(smsSingleRequest).toString(), Map.class);
        try {
